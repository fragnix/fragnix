module Fragnix.Primitive where
...
loadPrimitiveSymbols :: IO (Map ModuleName [Symbol])
loadPrimitiveSymbols = do
    fmap fromList (forM primitiveModules (\modulname -> do
        symbols <- readInterface (primitivePath modulname)
        return (modulname,symbols)))

primitivePath :: ModuleName -> FilePath
primitivePath modulname = "fragnix" </> "primitive_names" </> prettyPrint modulname
