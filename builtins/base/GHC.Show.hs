...
integerToString n0 cs0
    ...
    integerToString' n cs
        | n < 1000000000000000000  = jhead (fromInteger n) cs
        | otherwise = jprinth (jsplitf (1000000000000000000*1000000000000000000) n) cs
    ...
    -- Convert a number that has been split into digits in base 1000000000000000000^2
    ...
    jprinth (n:ns) cs =
        case n `quotRemInteger` 1000000000000000000 of
        (# q', r' #) ->
            let q = fromInteger q'
                r = fromInteger r'
    ...
    jprintb []     cs = cs
    jprintb (n:ns) cs = case n `quotRemInteger` 1000000000000000000 of
                        (# q', r' #) ->
                            let q = fromInteger q'
                                r = fromInteger r'
