...
-- We assume that 2 is 2 so that we can use more efficient code
    properFraction (F# x#)
      = case decodeFloat_Int# x# of
        (# m#, n# #) ->
            let m = I# m#
...
instance  RealFloat Float  where
    floatRadix _        =  2        -- from float.h
    floatDigits _       =  24     -- ditto
    floatRange _        =  ((-125), 128) -- ditto
...
instance  RealFloat Double  where
    floatRadix _        =  2        -- from float.h
    floatDigits _       =  53     -- ditto
    floatRange _        =  ((-1021), 1024) -- ditto
