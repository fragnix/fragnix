...
unsafeRead (Array ref) ix = do
    ...
    if (ix) < 0 || (ix) >= (cap) then error ("GHC.Event.Array." ++ ("unsafeRead") ++ ": bounds error, index " ++ show (ix) ++ ", capacity " ++ show (cap)) else
      withForeignPtr es $ \p ->
        peekElemOff p ix
...
unsafeWrite' (AC es _ cap) ix a = do
    if (ix) < 0 || (ix) >= (cap) then error ("GHC.Event.Array." ++ ("unsafeWrite'") ++ ": bounds error, index " ++ show (ix) ++ ", capacity " ++ show (cap)) else
      withForeignPtr es $ \p ->
        pokeElemOff p ix a
