...
-- NB: all code assumes GMP_LIMB_BITS == 64
...
isValidInteger# (Jp# bn)
    = isValidBigNat# bn `andI#` (bn `gtBigNatWord#` 0x7fffffffffffffff##)
isValidInteger# (Jn# bn)
    = isValidBigNat# bn `andI#` (bn `gtBigNatWord#` 0x8000000000000000##)
...
bitInteger i#
  | isTrue# (i# <# (64# -# 1#)) = S# (uncheckedIShiftL# 1# i#)
  | True = Jp# (bitBigNat i#)
{-# NOINLINE bitInteger #-}
...
testBitInteger (S# i#) n#
  | isTrue# (n# <# 64#) = isTrue# (((uncheckedIShiftL# 1# n#)
                                               `andI#` i#) /=# 0#)
  | True                          = isTrue# (i# <# 0#)
...
sizeofBigNat# (BN# x#)
    = sizeofByteArray# x# `uncheckedIShiftRL#` 3#
...
getSizeofMutBigNat# (MBN# x#) s =
    case getSizeofMutableByteArray# x# s of
        (# s', n# #) -> (# s', n# `uncheckedIShiftRL#` 3# #)

newBigNat# :: GmpSize# -> S s (MutBigNat s)
newBigNat# limbs# s =
    case newByteArray# (limbs# `uncheckedIShiftL#` 3#) s of
        (# s', mba# #) -> (# s', MBN# mba# #)
...
copyWordArray# src src_ofs dst dst_ofs len
  = copyByteArray# src (src_ofs `uncheckedIShiftL#` 3#)
                   dst (dst_ofs `uncheckedIShiftL#` 3#)
                   (len `uncheckedIShiftL#` 3#)
...
byteArrayToBigNat# ba# n0#
  ...
  | True = runS $ \s ->
      ...
      in go s''
  where
    (# baszq#, baszr# #) = quotRemInt# (sizeofByteArray# ba#) 8#
...
importBigNatFromAddr# addr len msbf = do
    ...
    unsafeFreezeBigNat# mbn
  where
    -- n = ceiling(len / 8), i.e. number of limbs required
    n# = (word2Int# len +# (8# -# 1#)) `quotInt#` 8#
...
absI# i# = (i# `xorI#` nsign) -# nsign
  where
    -- nsign = negateInt# (i# <# 0#)
    nsign = uncheckedIShiftRA# i# (64# -# 1#)
